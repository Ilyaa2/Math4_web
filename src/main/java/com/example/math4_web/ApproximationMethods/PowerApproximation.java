...
public class PowerApproximation extends ApproximationMethod {
    ...
    private List<Coordinates> preprocessing(List<Coordinates> list) {
        ...
        for (Coordinates coordinates : list) {
            Coordinates newCoord = new Coordinates(Math.log(coordinates.getX()), Math.log(coordinates.getY()));
    ...
    public Map<String, Double> calculate(List<Coordinates> list) {
        ...
        map.put("b", a);
        approximatingFunction = (x) -> map.get("a") * Math.pow(x, map.get("b"));
        return map;
    }
}
